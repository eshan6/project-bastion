...
Per-source, not per-domain — several sources share a domain (PIB, MoD
subpages on mod.gov.in) and we want explicit, auditable throttling.
"""
...
class RateLimiter:
    ...
    def wait(self, source_id: str, min_interval_seconds: float) -> float:
        """Block until permitted; return actual wait duration."""
