...
def load_all():
    ...
    out = {t: _load(t) for t in tables}
    # spares_consumption is OPTIONAL: predict_service.run_snapshot() already
    # branches on its presence ("spares_consumption absent — skipping spares
    # forecast"). Loading it unconditionally made that guard unreachable — a
    # materialized live_data dir missing the file crashed the whole prediction
    # service (and every /snapshot & /optimize call) instead of skipping one
    # forecaster. Load it only if the file exists.
    if (cfg.DATA_DIR / "spares_consumption.parquet").exists():
        out["spares_consumption"] = _load("spares_consumption")
    return out
