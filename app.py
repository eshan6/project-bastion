...
NEW (the live world):
  ...
  POST /advance   {"days": N, "token": <ADVANCE_TOKEN>} manual advance +
                  replan. Requires the ADVANCE_TOKEN Space secret; disabled
                  (403) when the secret is unset — world state is shared.

ROAD SPACE (v2.3 — wires stage4_planning/road_space.py into the service):
  GET /road_space?date=YYYY-MM-DD
        → {allotments:[RoadSpaceAllotment rows], diagnostics:{...}}
        Marshals the cached plans' vehicle sorties into convoy packets and
        assigns each packet a deconflicted entry time block per axis per
        dispatch day (CP-SAT, deterministic). Results are persisted as
        first-class ontology rows in <snapshot>/stage4/ and served from
        cache thereafter. This is the PDS 3 "road space management" layer.
...
def _road_space():
    """Import stage4_planning/road_space.py on first use. Raises 503 (not
    500) if the module cannot load, so the rest of the service stays up."""
    global _rsm_mod
    if _rsm_mod is None:
        try:
            import road_space as _r  # noqa: PLC0415
            _rsm_mod = _r
        except Exception as e:  # noqa: BLE001
            raise HTTPException(503, detail=f"road-space engine unavailable: {str(e)[-600:]}")
    return _rsm_mod
...
app = FastAPI(title="Bastion Model Service", version="2.4")
...
@app.get("/road_space")
async def road_space(date_: str | None = None, date: str | None = None):  # noqa: A002
    """Road Space Management (PDS 3): convoy packets + deconflicted entry
    time blocks for every plan in the snapshot. Deterministic; computed
    once per snapshot then served from the persisted parquet."""
    sim = _require_ready()
    d = pd.Timestamp(date or date_ or sim).date()
    await _ensure_snapshot(d)
    rsm = _road_space()
    snap = _snap_dir(d)
    try:
        res = await asyncio.to_thread(rsm.allot, str(snap))
    except HTTPException:
        raise
    except Exception as e:  # noqa: BLE001
        raise HTTPException(500, detail={"stage": "road_space",
                                          "date": d.isoformat(),
                                          "cause": str(e)[-1200:]})
    return {"allotments": _clean(res["allotments"]),
            "diagnostics": _clean(res["diagnostics"]),
            "sim_date": _status["sim_date"],
...
async def advance(req: AdvanceReq):
    # v2.4: /advance mutates SHARED world state for every viewer of the demo.
    # It now requires the admin token from the Space's ADVANCE_TOKEN secret
    # (Settings → Variables and secrets). If no secret is configured the
    # endpoint is disabled outright — fail closed, not open.
    expected = os.environ.get("ADVANCE_TOKEN", "")
    if not expected:
        raise HTTPException(403, detail="/advance is disabled: set the "
                            "ADVANCE_TOKEN secret on the Space to enable "
                            "manual world advancement")
    if not req.token or req.token != expected:
        raise HTTPException(403, detail="invalid or missing admin token")
    d = _require_ready()
