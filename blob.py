...
Content-addressed storage: blob_path is derived from sha256, so two
fetches that produce identical bytes share one file on disk.

Layout:
    {BLOB_ROOT}/{source_id}/{sha256[0:2]}/{sha256[2:4]}/{sha256}.{ext}

Why not S3 yet:
- Phase 1 is laptop-local. Dev velocity matters more than survivability.
- Add a nightly rsync to an external drive (cheap survivability).
- Migrate to S3-compatible (Cloudflare R2 free tier, or MinIO) when
  pilot scope demands it.
"""
...
# Falls back to mimetypes.guess_extension() and finally to '.bin'.
_EXT_OVERRIDES = {
    "text/html": ".html",
...
def _ext_for(content_type: str | None) -> str:
    if not content_type:
        return ".bin"
    # strip charset etc. e.g. "text/html; charset=utf-8" -> "text/html"
...
def write_blob(source_id: str, content: bytes, content_type: str | None) -> tuple[str, str, int]:
    ...
    Returns (blob_path, sha256_hex, size_bytes).
