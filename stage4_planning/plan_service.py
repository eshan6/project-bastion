...
Stage 3 snapshot dir, it:
  ...
  2. solves the 3-plan ε-constraint frontier (optimizer.solve_all v3.0:
     full_coverage / min_cost / min_exposure — see optimizer.py for why the
     v2 four-label set was retired)
...
def run_plan(snapshot_dir: str | Path,
    ...
    for obj in cfg.OBJECTIVES:
        ...
        plan_rows.append({
            ...
            "max_leg_risk": res["max_leg_risk"], "risky_sorties": res["risky_sorties"],
            "expected_arrived_tonnes": res["expected_arrived_tonnes"],
            "expected_loss_tonnes": res["expected_loss_tonnes"],
            "unserved_reachable_tonnes": res["unserved_reachable_tonnes"],
            "objective_tradeoff_tonnes": res["objective_tradeoff_tonnes"],
            "expected_shortfall_tonnes": res["expected_shortfall_tonnes"],
    ...
    for pc in plan_compare:
        print(f"    {pc['objective']:13s}  reach-cover {pc['coverage_reachable_pct']:5.1f}%  "
              f"convoys {pc['convoys']:3d}  risky {pc['risky_sorties']:3d}  ₹{pc['total_cost']:>11,.0f}  "
              f"exp-loss {pc['expected_loss_tonnes']:6.1f}t  "
              f"dropped {pc['objective_tradeoff_tonnes']:5.1f}t  "
...
def _interpretation(b: inputs.Bundle, plan_compare: list) -> list:
    ...
    # plan spread — the frontier is the decision
    ...
    if fc and mx:
        notes.append(
            ...
            f"'objective_tradeoff'), cutting convoys {fc['convoys']}→{mx['convoys']} "
            f"and expected in-transit loss "
            f"{fc['expected_loss_tonnes']:.1f}→{mx['expected_loss_tonnes']:.1f} t for "
            f"₹{fc['total_cost']-mx['total_cost']:,.0f} less. Tier-1 is identical in "
            f"every plan by construction. The commander chooses the commitment, not a label.")
    notes.append(
        "expected_loss_tonnes is computed at the planning-horizon path availability — "
        "a conservative exposure index for COMPARING plans, not a literal arrival "
        "forecast (convoys dispatch anticipatorily while roads are open). "
        "aggregate_risk (P(>=1 sortie disrupted)) is retained for schema continuity "
        "but saturates near 1.0 with ~80 winter sorties; use the tonnage-denominated "
